...
export class SNBTParser {
    ...
    parse(): NBTValue {
        ...
        if (this.position < this.input.length) {
            throw new SNBTParseError(`Unexpected character '${this.peek()}'`, this.position);
        }
    ...
    private parseCompound(): { [key: string]: NBTValue } {
        ...
        while (true) {
            ...
            if (this.peek() === '}') {
                ...
                throw new SNBTParseError(`Expected ',' or '}' in compound`, this.position);
            }
        }
        ...
        if (this.peek() === 'B' || this.peek() === 'I' || this.peek() === 'L') {
            ...
            while (true) {
                ...
                if (this.peek() === ']') {
                    ...
                    throw new SNBTParseError(`Expected ',' or ']' in typed array`, this.position);
                }
            }
    ...
    private parseUnquotedString(): string {
        ...
        if (!this.isValidUnquotedString(result)) {
            throw new SNBTParseError(`Invalid unquoted string: ${result}`, this.position - result.length);
        }
    ...
    private parseNumberFromString(numStr: string, suffix?: string): number | bigint {
        if (suffix === 'l') {
    ...
    private createTypedArray(type: string, values: (number | bigint)[]): Int8Array | Int32Array | BigInt64Array {
        switch (type.toLowerCase()) {
            ...
            case 'l':
                return new BigInt64Array(values.map(v => typeof v === 'bigint' ? v : BigInt(v)));
            default:
    ...
    private checkDepth(): void {
        ...
        if (this.depth > this.options.maxDepth) {
            throw new SNBTParseError(`Maximum depth exceeded: ${this.options.maxDepth}`, this.position);
        }
    }
    ...
    private consume(expected: string): void {
        if (this.peek() !== expected) {
            throw new SNBTParseError(`Expected '${expected}', got '${this.peek()}'`, this.position);
        }
        this.advance();
    }
