...
describe("calculator store forge tracker", () => {
  ...
  it("starts a partially elapsed job and enforces global slot capacity", () => {
    ...
    expect(store.startForgeJob("TARGET", "PART", 100, 40, 2)).toBe(true);
    expect(store.startForgeJob("OTHER", "OTHER_PART", 100, 100, 2)).toBe(false);
  ...
  it("keeps a completed job ready until it is collected", () => {
    const store = useCalculatorStore.getState();
    expect(store.startForgeJob("TARGET", "PART", 10, 10, 1)).toBe(true);
  ...
  it("cancels without increasing completed quantity", () => {
    const store = useCalculatorStore.getState();
    store.startForgeJob("TARGET", "PART", 10, 10, 1);
    const jobId = useCalculatorStore.getState().activeForgeJobs[0]?.id;
    if (!jobId) return;
  ...
  it("resets only the selected plan and its jobs", () => {
    ...
    store.startForgeJob("TARGET", "PART", 10, 10, 5);
    store.startForgeJob("OTHER", "OTHER_PART", 10, 10, 5);
      ...
      return JSON.stringify({
        ...
        plans: {
          TARGET: {
            ...
            completedByItem: { PART: 2.8, INVALID: "3" },
          },
        },
        activeJobs: [
          {
